...
import {
  ...
  Info,
  PackagePlus,
} from 'lucide-react'; // Renamed CalendarIcon, Added PackagePlus
import type {
  Sprint,
  SprintPlanning,
  Task,
  ...
  HolidayCalendar,
  Team,
} from '@/types/sprint-data'; // Added HolidayCalendar, Team
import {
  initialSprintPlanning,
  taskStatuses,
  predefinedRoles,
  taskPriorities,
} from '@/types/sprint-data'; // Added taskPriorities
...
const calculateEndDateSkippingWeekends = (
  startDate: Date,
  workingDays: number
): Date => {
...
export default function PlanningTab({
  ...
  const [selectedBacklogIds, setSelectedBacklogIds] = useState<Set<string>>(
    new Set()
  ); // State for selected backlog item IDs
  ...
  const findTeamLead = useCallback(
    (memberName: string | undefined): string | undefined => {
  ...
  const handleAddSelectedBacklogItems = () => {
    ...
    toast({
      title: 'Items Added',
      description: `${itemsToAdd.length} backlog item(s) added to the sprint plan.`,
    });
  ...
  const finalizeTasks = (
    ...
    taskRows.forEach((row, index) => {
      ...
      if (
        qaEstimatedTime &&
        parseEstimatedTimeToDays(qaEstimatedTime) === null
      ) {
        errors.push(
      ...
      finalTasks.push({
        id:
          row.id ||
          `task_${selectedSprintNumber ?? 'new'}_${taskType === 'new' ? 'n' : 's'}_${Date.now()}_${index}`,
  ...
  const handleCreateAndSaveNewSprint = () => {
    ...
    if (formErrors.length > 0 || taskErrors.length > 0) {
      toast({
        variant: 'destructive',
        title: 'Validation Error',
        description: [...formErrors, ...taskErrors].join('\n'),
      });
      return;
    }
  ...
  const handleStartSprint = () => {
    ...
    if (alreadyActiveSprint) {
      toast({
        variant: 'destructive',
        title: 'Error',
        description: `Sprint ${alreadyActiveSprint.sprintNumber} is already active. Complete or replan it first.`,
      });
      return;
    }
      ...
      <Popover>
        ...
        <PopoverContent className="w-auto p-0">
          <Calendar
            ...
            selected={dateValue}
            onSelect={(date) =>
              handleTaskDateChange(type, row._internalId, field, date)
            }
            initialFocus
          />
        </PopoverContent>
  ...
  const renderTaskTable = (
    ...
    <div className="overflow-x-auto">
      ...
      <div className="min-w-[1150px] space-y-4">
        {' '}
        {/* Adjusted min-width */}
        {/* Grid column definitions */}
        <div className="hidden grid-cols-[100px_70px_100px_100px_100px_150px_150px_120px_100px_40px] items-center gap-x-2 border-b pb-2 md:grid">
          {' '}
          {/* Reduced Ticket # width */}
        ...
        <div className="space-y-4 md:space-y-2">
          {taskRows.map((row) => (
            ...
            <div
              key={row._internalId}
              className="grid grid-cols-2 items-start gap-x-2 gap-y-2 border-b pb-4 last:border-b-0 md:grid-cols-[100px_70px_100px_100px_100px_150px_150px_120px_100px_40px] md:border-none md:pb-0"
            >
              {' '}
              {/* Reduced Ticket # width */}
              {/* Ticket Number */}
  ...
  const renderPlanningForm = (disabled: boolean) => (
    <>
      ...
      <Card>
        ...
        <CardFooter>
          <p className="text-xs text-muted-foreground">
            * Required field: Ticket #, Start Date. Other fields optional but
            recommended for planning/timeline.
          </p>
        </CardFooter>
      </Card>
      ...
      <Card>
        ...
        <CardFooter>
          <p className="text-xs text-muted-foreground">
            * Required field: Ticket #, Start Date.
          </p>
        </CardFooter>
      </Card>
                ...
                <Popover>
                  ...
                  <PopoverContent className="w-auto p-0">
                    <Calendar
                      ...
                      onSelect={(date) =>
                        handleNewSprintFormChange('startDate', date)
                      }
                      initialFocus
                    />
                  </PopoverContent>
          ...
          <CardContent>
            {sprints.length === 0 ? (
              <p className="py-8 text-center text-muted-foreground">
                No existing sprints found. Plan a new sprint above or add past
                sprints in the 'Entry' tab.
              </p>
            ) : (
              <div className="overflow-x-auto">
                <Table>
                    ...
                    {sprints
                      ...
                      .map((sprint) => (
                        <TableRow
                          ...
                          className={cn(
                            selectedSprintNumber === sprint.sprintNumber &&
                              'bg-muted'
                          )}
                        >
