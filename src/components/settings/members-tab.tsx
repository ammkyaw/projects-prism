...
export default function MembersTab({
  ...
  useEffect(() => {
    ...
    const sortMembers = (m: Member[]) =>
      m.slice().sort((a, b) => a.name.localeCompare(b.name));
  ...
  const handleSave = () => {
    ...
    memberRows.forEach((row, index) => {
      ...
      if (
        holidayCalendarId &&
        !holidayCalendars.some((cal) => cal.id === holidayCalendarId)
      ) {
  ...
  return (
    <Card>
        ...
        <div className="flex items-center justify-between">
          <div>
            <CardTitle className="flex items-center gap-2">
              <Users className="h-5 w-5 text-primary" /> Project Members:{' '}
              {projectName}
            </CardTitle>
            <CardDescription>
              Add or edit team members, their roles, and assign holiday
              calendars.
            </CardDescription>
          </div>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="hidden grid-cols-[1fr_1fr_1fr_40px] items-center gap-x-3 border-b pb-2 md:grid">
        ...
        <div className="space-y-4 md:space-y-2">
          {memberRows.map((row) => (
            <div
              key={row._internalId}
              className="grid grid-cols-2 items-start gap-x-3 gap-y-2 border-b pb-4 last:border-b-0 md:grid-cols-[1fr_1fr_1fr_40px] md:border-none md:pb-0"
            >
              {/* Name */}
              <div className="col-span-2 md:col-span-1">
                <Label
                  htmlFor={`name-${row._internalId}`}
              ...
              <div className="col-span-1 md:col-span-1">
                <Label
                  htmlFor={`calendar-${row._internalId}`}
                  className="text-xs font-medium md:hidden"
                >
                  <CalendarDays className="mr-1 inline h-3 w-3" />
                  Holiday Calendar
                ...
                <Select
                  ...
                  onValueChange={(value) =>
                    handleCalendarChange(row._internalId, value)
                  }
                  disabled={holidayCalendars.length === 0}
                >
                  <SelectTrigger
                    id={`calendar-${row._internalId}`}
                    className="h-9"
                  >
