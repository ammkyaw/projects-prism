...
import { storyPointScaleOptions, taskTypes as predefinedTaskTypes, taskStatuses as predefinedTicketStatuses } from '@/types/sprint-data';
...
export default function ConfigTab({
  ...
  useEffect(() => {
    ...
    const originalTicketStatuses = JSON.stringify(initialCustomTicketStatuses || []);
  ...
  }, [selectedScale, customTaskTypes, customTicketStatuses, initialStoryPointScale, initialCustomTaskTypes, initialCustomTicketStatuses]);


  const handleAddCustomTaskType = () => {
    const trimmedType = newCustomTaskType.trim();
    if (trimmedType && !customTaskTypes.includes(trimmedType) && !predefinedTaskTypes.includes(trimmedType as any)) {
      setCustomTaskTypes((prev) => [...prev, trimmedType].sort());
      setNewCustomTaskType('');
    } else if (predefinedTaskTypes.includes(trimmedType as any)) {
        toast({ variant: 'destructive', title: 'Error', description: `"${trimmedType}" is a predefined task type.`});
    } else if (customTaskTypes.includes(trimmedType)) {
        toast({ variant: 'destructive', title: 'Error', description: `Task type "${trimmedType}" already exists.`});
    }
  };
  ...
  const handleAddCustomTicketStatus = () => {
    ...
    if (trimmedStatus && !customTicketStatuses.includes(trimmedStatus) && !predefinedTicketStatuses.includes(trimmedStatus as any)) {
      ...
      setNewCustomTicketStatus('');
        ...
        toast({ variant: 'destructive', title: 'Error', description: `"${trimmedStatus}" is a predefined ticket status.`});
    } else if (customTicketStatuses.includes(trimmedStatus)) {
        toast({ variant: 'destructive', title: 'Error', description: `Ticket status "${trimmedStatus}" already exists.`});
    }
  };
  ...
  const handleSave = () => {
    ...
    if (customTaskTypes.some(type => !type.trim())) {
        toast({ variant: 'destructive', title: 'Error', description: 'Custom task types cannot be empty.' });
        return;
    }
    if (customTicketStatuses.some(status => !status.trim())) {
        toast({ variant: 'destructive', title: 'Error', description: 'Custom ticket statuses cannot be empty.' });
        return;
    }
        ...
        <div className="flex items-center justify-between">
          <div>
            <CardTitle className="flex items-center gap-2">
              <Settings2 className="h-5 w-5 text-primary" /> Project Configurations: {projectName}
            </CardTitle>
            <CardDescription>
              Customize story point scales, task types, and ticket statuses for this project.
            </CardDescription>
          </div>
      ...
      <CardContent className="space-y-8">
        ...
        <section>
          <h3 className="mb-3 text-lg font-semibold flex items-center gap-2">
            <Scaling className="h-5 w-5 text-muted-foreground" /> Story Point Scale
          </h3>
          <div className="max-w-sm space-y-1">
            <Label htmlFor="story-point-scale">Estimation Scale</Label>
            <Select
              value={selectedScale}
              onValueChange={(value) => setSelectedScale(value as StoryPointScale)}
            >
        ...
        <section>
          <h3 className="mb-3 text-lg font-semibold">Custom Task Types</h3>
           <p className="text-sm text-muted-foreground mb-2">Predefined types: {predefinedTaskTypes.join(', ')}.</p>
          <div className="flex items-end gap-2 mb-3">
            <div className="flex-grow space-y-1">
              ...
              <Input
                ...
                onKeyDown={(e) => e.key === 'Enter' && handleAddCustomTaskType()}
              />
            </div>
            <Button onClick={handleAddCustomTaskType} variant="outline" size="icon" aria-label="Add Task Type">
              <PlusCircle className="h-5 w-5" />
            </Button>
          </div>
          {customTaskTypes.length > 0 && (
            <div className="space-y-2 rounded-md border p-3">
              <h4 className="text-sm font-medium text-muted-foreground">Your Custom Task Types:</h4>
        ...
        <section>
          <h3 className="mb-3 text-lg font-semibold">Custom Ticket Statuses</h3>
           <p className="text-sm text-muted-foreground mb-2">Predefined statuses: {predefinedTicketStatuses.filter(s => s !== null).join(', ')}.</p>
          <div className="flex items-end gap-2 mb-3">
            <div className="flex-grow space-y-1">
              ...
              <Input
                ...
                placeholder="e.g., Ready for UAT, Deploy"
                 onKeyDown={(e) => e.key === 'Enter' && handleAddCustomTicketStatus()}
              />
            </div>
            <Button onClick={handleAddCustomTicketStatus} variant="outline" size="icon" aria-label="Add Ticket Status">
              <PlusCircle className="h-5 w-5" />
            </Button>
          </div>
          {customTicketStatuses.length > 0 && (
             <div className="space-y-2 rounded-md border p-3">
                <h4 className="text-sm font-medium text-muted-foreground">Your Custom Ticket Statuses:</h4>
                <div className="flex flex-wrap gap-2">
