...
type SortKey =
  ...
  | 'movedToSprint'
  | 'backlogId'
  | 'initiator'; // Added backlogId, initiator
    ...
    <>
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            View backlog items that have been moved to sprints for project '
            {projectName}'. Click headers to sort.
          </CardDescription>
        </CardHeader>
        <CardContent>
          {sortedHistory.length === 0 ? (
            <div className="flex min-h-[200px] flex-col items-center justify-center rounded-md border-2 border-dashed p-6">
              ...
              <p className="text-muted-foreground">
                No backlog items have been moved to sprints yet.
              </p>
            </div>
          ) : (
                ...
                <TableBody>
                  {sortedHistory.map((task) => (
                    <TableRow key={task.id}>
                      ...
                      {/* Created Date */}
                      <TableCell>
                        {task.createdDate && isValid(parseISO(task.createdDate))
