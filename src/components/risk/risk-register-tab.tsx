...
interface RiskRegisterTabProps {
  ...
  onSaveRisk: (newRisk: RiskItem) => void; // Callback to save a new risk
...
export default function RiskRegisterTab({
  ...
  const handleSaveRiskItem = (risk: RiskItem) => {
    if (editingRisk) {
      onUpdateRisk({ ...risk, id: editingRisk.id }); // Ensure ID is preserved for updates
      toast({ title: 'Risk Updated', description: `Risk "${risk.title}" has been updated.` });
    } else {
      onSaveRisk(risk);
      // Toast is handled in useSettingsActions
    }
  ...
  return (
    <>
      ...
      <RegisterRiskModal
        ...
        // Pass editingRisk to prefill the form if an item is being edited
        // The modal itself will need to handle this `initialData` prop
