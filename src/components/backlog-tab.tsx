...
export default function BacklogTab({
  ...
  const [editingDepsTaskId, setEditingDepsTaskId] = useState<string | null>(
    null
  ); // ID of the task whose dependencies are being edited
  ...
  const rowRefs = useRef<Map<string, React.RefObject<HTMLDivElement>>>(
    new Map()
  );
  ...
  useEffect(() => {
    ...
    if (mappedBacklog.length === 0) {
      ...
      setBacklogRows([
        { ...newRow, ref: rowRefs.current.get(newRow._internalId) },
      ]);
    }
  ...
  useEffect(() => {
    const cleanBacklog = (
      tasks: Task[]
    ): Omit<Task, 'id' | 'status' | 'createdDateObj' | '_internalId'>[] =>
      tasks
        .map(
          ({
            id,
    ...
    const currentBacklogString = JSON.stringify(
      cleanBacklog(
        backlogRows.filter(
          (row) => row.backlogId?.trim() || row.title?.trim() || row.storyPoints
        )
      )
    );
  ...
  const handleInputChange = (
    ...
    field: keyof Omit<
      BacklogRow,
      'id' | '_internalId' | 'dependsOn' | 'createdDateObj' | 'ref'
    >,
  ...
  const handleSelectChange = (
    internalId: string,
    field: 'taskType' | 'priority' | 'initiator',
    value: string
  ) => {
  ...
  const handleSave = () => {
    ...
    backlogRows.forEach((row, index) => {
      ...
      const invalidDeps = dependsOn.filter(
        (depId) => !existingBacklogIds.has(depId)
      );
  ...
  const potentialDependencies = useMemo(() => {
    ...
    return backlogRows
      .filter(
        (row) => row._internalId !== editingDepsTaskId && row.backlogId?.trim()
      ) // Exclude self and rows without ID
      .map((row) => ({
        id: row.backlogId!,
        title: row.title || `Item ${row.backlogId}`,
      })); // Use backlogId as the ID
  ...
  return (
    <>
          ...
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="flex items-center gap-2">
                <Package className="h-5 w-5 text-primary" /> Project Backlog:{' '}
                {projectName}
              </CardTitle>
              <CardDescription>
                Manage tasks that are not yet planned for a specific sprint.
                Add, edit, prioritize, and detail backlog items.
              </CardDescription>
            </div>
        ...
        <CardContent className="space-y-4">
          <div className="overflow-x-auto" ref={backlogContainerRef}>
            <div className="min-w-[1200px] space-y-4">
              ...
              <div className="sticky top-0 z-10 hidden grid-cols-[100px_1fr_120px_120px_120px_100px_100px_80px_40px] items-center gap-x-3 border-b bg-card pb-2 md:grid">
                {' '}
                {/* Added sticky header */}
                <Label className="flex items-center gap-1 text-xs font-medium text-muted-foreground">
                  <ArrowUpDown className="h-3 w-3" /> Backlog ID*
                </Label>
              ...
              <div className="space-y-4 md:space-y-2">
                {backlogRows.map((row) => (
                  <div
                    ...
                    className="grid grid-cols-2 items-start gap-x-3 gap-y-2 border-b pb-4 transition-colors duration-1000 last:border-b-0 md:grid-cols-[100px_1fr_120px_120px_120px_100px_100px_80px_40px] md:border-none md:pb-0" // Add transition for highlight
                  >
                    {/* Backlog ID */}
                    <div className="relative col-span-1 md:col-span-1">
                      <Label
                        htmlFor={`backlog-id-${row._internalId}`}
                      ...
                      <Input
                        ...
                        className={cn(
                          'h-9',
                          row.backlogId?.trim() && 'text-transparent'
                        )} // Hide text if ID exists and link is shown
                        required
                      />
                      {row.backlogId?.trim() && (
                        <button
                          ...
                          className="absolute inset-0 z-10 flex cursor-pointer items-center border-none bg-transparent pl-3 text-sm font-medium text-primary underline hover:text-primary/80"
                    ...
                    <div className="col-span-2 md:col-span-1">
                      <Label
                        htmlFor={`backlog-title-${row._internalId}`}
                        className="text-xs font-medium md:hidden"
                      >
                        Title*
                      </Label>
                    ...
                    <div className="col-span-1 md:col-span-1">
                      <Label
                        htmlFor={`backlog-type-${row._internalId}`}
                        className="text-xs font-medium md:hidden"
                      >
                        Task Type*
                      </Label>
                      <Select
                        ...
                        onValueChange={(value) =>
                          handleSelectChange(row._internalId, 'taskType', value)
                        }
                      >
                        <SelectTrigger
                          id={`backlog-type-${row._internalId}`}
                          className="h-9"
                        >
                    ...
                    <div className="col-span-1 md:col-span-1">
                      <Label
                        htmlFor={`backlog-initiator-${row._internalId}`}
                        className="text-xs font-medium md:hidden"
                      >
                        Initiator
                      </Label>
                      ...
                      >
                        <SelectTrigger
                          id={`backlog-initiator-${row._internalId}`}
                          className="h-9"
                        >
                    ...
                    <div className="col-span-1 md:col-span-1">
                      <Label
                        htmlFor={`backlog-created-${row._internalId}`}
                        className="text-xs font-medium md:hidden"
                      >
                        Created Date*
                      </Label>
                    ...
                    <div className="col-span-1 md:col-span-1">
                      <Label
                        htmlFor={`backlog-priority-${row._internalId}`}
                        className="text-xs font-medium md:hidden"
                      >
                        Priority*
                      </Label>
                      <Select
                        ...
                        onValueChange={(value) =>
                          handleSelectChange(row._internalId, 'priority', value)
                        }
                      >
                        <SelectTrigger
                          id={`backlog-priority-${row._internalId}`}
                          className="h-9"
                        >
                    ...
                    <div className="col-span-2 self-center md:col-span-1">
                      <Label
                        htmlFor={`backlog-deps-${row._internalId}`}
                        className="text-xs font-medium md:hidden"
                      >
                        Dependencies
                      </Label>
                    ...
                    {/* Actions Cell */}
                    <div className="col-span-2 flex items-center justify-center gap-1 md:col-span-1">
                      <Button
                        variant="ghost"
                        size="icon"
                        className="h-7 w-7"
                        disabled={
                          availableSprints.length === 0 || !row.id?.trim()
                        } // Disable if no sprints or row is not saved yet
        ...
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              {backlogRows.find((r) => r.id === movingTaskId)?.backlogId}' to.
            </DialogDescription>
          </DialogHeader>
          <div className="py-4">
      ...
      <SelectDependenciesDialog
        ...
        currentDependencies={
          backlogRows.find((row) => row._internalId === editingDepsTaskId)
            ?.dependsOn ?? []
        }
