...
export default function ManualInputForm({ onSubmit }: ManualInputFormProps) {
  ...
  const [pasteData, setPasteData] = useState<string>(''); // For pasting TSV/CSV
 ...
 const processPastedData = () => {
    ...
    const expectedHeaders = ['SprintNumber', 'Date', 'Developer', 'StoryPointsCompleted', 'DayOfSprint', 'TotalSprintPoints', 'TotalDaysInSprint'];
    const newRows: ManualEntryRow[] = [];
    let headerLine = '';
    let dataLines: string[] = [];

    // Check if the first line looks like a header
    const firstLineCols = lines[0].split('\t'); // Assuming TSV, adjust if CSV is common
     if (expectedHeaders.every(header => firstLineCols.some(col => col.trim().toLowerCase() === header.toLowerCase()))) {
         headerLine = lines[0];
         dataLines = lines.slice(1);
     } else {
         // Assume no header provided, use expected headers implicitly
         dataLines = lines;
     }

     const headers = headerLine ? headerLine.split('\t').map(h => h.trim()) : expectedHeaders;
     const headerMap: { [key: string]: number } = {};
     headers.forEach((h, i) => {
        // Find the canonical header name (case-insensitive)
     ...
     if (missingHeaders.length > 0 && headerLine) { // Only throw error if headers were provided but are incomplete
          toast({ variant: "destructive", title: "Error", description: `Missing expected columns in pasted data: ${missingHeaders.join(', ')}` });
         return;
     }
    ...
    dataLines.forEach((line, index) => {
        ...
        const values = line.split('\t'); // Assuming TSV
         if (values.length < expectedHeaders.length && !headerLine) {
             console.warn(`Skipping pasted line ${index + (headerLine ? 2 : 1)}: Not enough columns.`);
        ...
        const newRow: Partial<ManualEntryRow> = { id: Date.now() + index };
         ...
         expectedHeaders.forEach(header => {
            const colIndex = headerMap[header];
             const value = colIndex !== undefined ? (values[colIndex] || '').trim() : ''; // Use mapped index or default if no header/mapping
             (newRow as any)[header] = value;
         });


        // Basic check if essential fields might be empty (can be improved)
    ...
    // Replace existing rows or append, depending on desired behavior.
    // Here, we replace:
    setRows(newRows);
 ...
 const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    rows.forEach((row, index) => {
        ...
        const date = row.date.trim(); // Basic validation: check if it's a valid date format? YYYY-MM-DD
        ...
        if (!developerPoints[developer][date]) developerPoints[developer][date] = 0;
        developerPoints[developer][date] += points;
    });
    ...
    // Finalize burndown calculations (carry over values for days with no activity)
    ...
    <div className="space-y-6">
       ...
       <Card>
          <CardHeader>
              ...
              <CardDescription>Paste tab-separated data (e.g., from Excel/Sheets). Include headers or ensure columns are in the correct order: SprintNumber, Date (YYYY-MM-DD), Developer, StoryPointsCompleted, DayOfSprint, TotalSprintPoints, TotalDaysInSprint.</CardDescription>
          </CardHeader>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Manual Sprint Data Entry</CardTitle>
           <CardDescription>
                Enter sprint task completion data row by row. All fields are required. Ensure consistency for TotalSprintPoints and TotalDaysInSprint within the same sprint.
           </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
             {/* Table Header */}
              <div className="hidden md:grid grid-cols-[auto_1fr_1fr_1fr_1fr_1fr_1fr_auto] gap-2 items-center pb-2 border-b">
            ...
            <div className="space-y-3">
              {rows.map((row, index) => (
                 <div key={row.id} className="grid grid-cols-1 md:grid-cols-[auto_1fr_1fr_1fr_1fr_1fr_1fr_auto] gap-2 items-center">
                    {/* Inputs with mobile labels */}
                   <div>
