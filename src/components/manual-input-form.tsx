...
import { addDays, format, parseISO, isPast, isValid } from 'date-fns'; // Import isPast and isValid
...
interface ManualInputFormProps {
  ...
  parseSprintData: (jsonData: any[]) => SprintData; // Keep parser for paste functionality if re-enabled
}
...
export default function ManualInputForm({ onSubmit, initialData = [] }: ManualInputFormProps) {
 ...
 const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    rows.forEach((row, index) => {
        ...
        if (!/^\d{4}-\d{2}-\d{2}$/.test(startDateStr) || !isValid(parseISO(startDateStr))) rowErrors.push("Invalid Start Date (use YYYY-MM-DD)");
      ...
      <Card>
        <CardHeader>
          <CardTitle>Manual Sprint Data Entry</CardTitle>
           <CardDescription>
                Enter or edit historical sprint data row by row. Required fields are marked. Dates must be YYYY-MM-DD. Sprints are initially 'Planned' or 'Completed' based on end date. Use the 'Planning' tab for new sprints.
           </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex justify-between items-center pt-4">
               ...
               <Button type="submit">Save Sprints</Button>
            </div>
          </form>
        </CardContent>
         <CardFooter>
