...
export default function DeveloperEffortChart({
  sprintData,
  members,
  selectedDeveloperId,
  selectedSprintNumber,
}: DeveloperEffortChartProps) {
  const { chartData, chartConfig, noDataMessage, xAxisDataKey } =
    useMemo(() => {
      if (
        !sprintData ||
        !sprintData.sprints ||
        ...
        return {
          chartData: [],
          chartConfig: {},
          noDataMessage: 'No sprint data available.',
          xAxisDataKey: 'name',
        };
      }
      if (!members || members.length === 0) {
        return {
          chartData: [],
          chartConfig: {},
          noDataMessage: 'No members found in the project.',
          xAxisDataKey: 'name',
        };
      }

      const softwareEngineers = members.filter(
        (m) => m.role === 'Software Engineer'
      );
      ...
      if (selectedDeveloperId && selectedDeveloperId !== 'all') {
        ...
        if (!developer) {
          return {
            chartData: [],
            chartConfig: {},
            noDataMessage: `Developer with ID ${selectedDeveloperId} not found.`,
            xAxisDataKey: 'sprintName',
          };
        }
        ...
        const specificDevChartData = completedSprints.map((sprint) => {
          ...
          const points = tasks.reduce((sum, task) => {
            if (
              task.status === 'Done' &&
              task.assignee === developer.name &&
              task.storyPoints != null
            ) {
              // Check if storyPoints is not null/undefined
        ...
        if (specificDevChartData.length === 0) {
          return {
            chartData: [],
            chartConfig: {},
            noDataMessage: `No completed sprints found for this developer.`,
            xAxisDataKey: 'sprintName',
          };
        }
        if (specificDevChartData.every((d) => d.points === 0)) {
          return {
            chartData: [],
            chartConfig: {},
            noDataMessage: `No completed story points by ${developer.name} in the last 10 completed sprints.`,
            xAxisDataKey: 'sprintName',
          };
        }
        ...
        return {
          chartData: specificDevChartData,
      ...
      // --- All Developers View (Selected Sprint) ---
        ...
        selectedSprintNumber &&
        (selectedDeveloperId === null || selectedDeveloperId === 'all')
      ) {
        // Adjusted condition
        const selectedSprint = sprintData.sprints.find(
        ...
        if (!selectedSprint) {
          return {
            chartData: [],
            chartConfig: {},
            noDataMessage: `Sprint ${selectedSprintNumber} not found.`,
            xAxisDataKey: 'developerName',
          };
        }
        if (softwareEngineers.length === 0) {
          return {
            ...
            chartConfig: {},
            noDataMessage:
              'No Software Engineers found in the project for this sprint.',
            xAxisDataKey: 'developerName',
          };
        }

        const tasksInSelectedSprint: Task[] = [
          ...(selectedSprint.planning?.newTasks || []),
          ...(selectedSprint.planning?.spilloverTasks || []),
        ];
        const completedTasks = tasksInSelectedSprint.filter(
          (task) =>
            task.status === 'Done' &&
            task.assignee &&
            ...
            members.some(
              (m) => m.name === task.assignee && m.role === 'Software Engineer'
            ) // Ensure assignee is an SE
        );
        ...
        if (completedTasks.length === 0) {
          return {
            chartData: [],
            chartConfig: {},
            noDataMessage: `No completed story points by developers in Sprint ${selectedSprintNumber}.`,
            xAxisDataKey: 'developerName',
          };
        }

        const dataByDeveloper: {
          [devName: string]: { developerName: string } & {
        ...
        completedTasks.forEach((task) => {
          ...
          if (dataByDeveloper[assigneeName]) {
            ...
            dataByDeveloper[assigneeName][ticketKey] =
              (dataByDeveloper[assigneeName][ticketKey] || 0) + points;
        ...
        if (
          allDevsChartData.every((devData) =>
            Object.keys(devData)
              .filter((k) => k !== 'developerName')
              .every((ticketKey) => (devData[ticketKey] || 0) === 0)
          )
        ) {
          return {
            chartData: [],
        ...
        return {
          chartData: allDevsChartData,
          chartConfig: allDevsChartConfig,
          noDataMessage: null,
          xAxisDataKey: 'developerName',
        };
      }
      // --- Default Case (e.g., no sprint selected for "All Developers") ---
        ...
        const activeSprint = sprintData.sprints.find(
          (s) => s.status === 'Active'
        );
        if (
          activeSprint &&
          ...
          if (softwareEngineers.length === 0) {
            return {
              chartData: [],
              chartConfig: {},
              noDataMessage: 'No Software Engineers found.',
              xAxisDataKey: 'developerName',
            };
          }
          const tasksInActiveSprint: Task[] = [
            ...(activeSprint.planning?.newTasks || []),
            ...(activeSprint.planning?.spilloverTasks || []),
          ];
          const completedTasks = tasksInActiveSprint.filter(
            (task) =>
              task.status === 'Done' &&
              task.assignee &&
              task.storyPoints != null &&
              ...
              members.some(
                (m) =>
                  m.name === task.assignee && m.role === 'Software Engineer'
              )
          );
          if (completedTasks.length === 0) {
            return {
              chartData: [],
              chartConfig: {},
              noDataMessage: `No completed story points by developers in the active sprint (Sprint ${activeSprint.sprintNumber}).`,
              xAxisDataKey: 'developerName',
            };
          }
          const dataByDeveloper: {
          ...
          completedTasks.forEach((task) => {
            ...
            if (dataByDeveloper[assigneeName]) {
              ...
              if (!activeSprintChartConfig[ticketKey]) {
                activeSprintChartConfig[ticketKey] = {
                  label: ticketKey,
                  color: getColorForString(ticketKey, baseChartColors),
                };
              }
            }
          });
          ...
          return {
            chartData: allDevsChartData,
            chartConfig: activeSprintChartConfig,
            noDataMessage: null,
            xAxisDataKey: 'developerName',
          };
        } else {
          return {
            ...
            chartConfig: {},
            noDataMessage:
              'Select a developer or ensure an active sprint exists to view contributions.',
            xAxisDataKey: 'name',
          };
        }
      }
      ...
      <ResponsiveContainer width="100%" height="100%">
        ...
        >
          ...
          {selectedDeveloperId && selectedDeveloperId !== 'all' ? (
            ...
            <Bar
              dataKey="points"
              fill={chartConfig.points?.color || baseChartColors[0]}
              radius={4}
              barSize={20}
              name={chartConfig.points?.label?.toString()}
            /> // Use label for name
          ) : (
