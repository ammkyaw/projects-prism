...
export default function AddTeamMemberDialog({
  ...
  const availableMembers = allMembers.filter(
    (member) => !currentTeamMembers.includes(member.id)
  );
      ...
      <DialogContent className="sm:max-w-[450px]">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Users className="h-5 w-5 text-primary" /> Add Members to Team
          </DialogTitle>
          <DialogDescription>
            Select members from the project list to add to this team. Already
            added members are not shown here.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="py-4">
          {availableMembers.length === 0 ? (
            <p className="py-4 text-center text-sm italic text-muted-foreground">
              All project members are already in this team, or no members exist
              in the project.
            </p>
          ) : (
