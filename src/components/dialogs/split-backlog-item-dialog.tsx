...
const createEmptySplitTaskRow = (
  ...
  return {
    ...
    title:
      `${baseTask.title} (${suffix.toUpperCase()})` ??
      `Split Task ${suffix.toUpperCase()}`,
...
export default function SplitBacklogItemDialog({
  ...
  const handleAddSplitTaskRow = () => {
    ...
    setSplitTaskRows((prev) => [
      ...prev,
      createEmptySplitTaskRow(originalTask, nextSuffix),
    ]);
  };
    ...
    setSplitTaskRows((prevRows) => {
      ...
      if (newRows.length === 0) {
        toast({
          variant: 'destructive',
          title: 'Error',
          description: 'Cannot remove the last split item.',
        });
        return prevRows;
      }
  ...
  const handleInputChange = (
    internalId: string,
    field: keyof Omit<SplitTaskRow, '_internalId' | 'id' | 'storyPoints'>, // Exclude storyPoints
  ...
  const handleConfirm = () => {
    ...
    if (splitTaskRows.length === 0) {
      toast({
        ...
        description: 'At least one split task is required.',
      });
      return;
    }
    ...
    splitTaskRows.forEach((row, index) => {
      ...
      if (!backlogId)
        rowErrors.push(
          `Split Task ${index + 1}: Backlog ID generation failed.`
        ); // Should not happen
      ...
      finalSplitTasks.push({
        ...
        storyPoints: storyPointsValue, // Use parsed value
        ...
        status: null,
        startDate: null,
        splitFromId: null, // Will be set in useBacklogActions
        mergeEventId: null,
        severity: row.severity ?? null, // Carry over severity if present
      });
    });
    ...
    if (finalSplitTasks.length === 0) {
      toast({
        variant: 'destructive',
        title: 'Validation Error',
        description: 'No valid split tasks were created.',
      });
      return;
    }
      ...
      <DialogContent className="sm:max-w-2xl">
        ...
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Split className="h-5 w-5 text-primary" /> Split Backlog Item:{' '}
            {originalTask.backlogId}
          </DialogTitle>
          <DialogDescription>
            Divide '{originalTask.title}' into smaller tasks. IDs will be
            auto-generated ({originalTask.backlogId}-a, {originalTask.backlogId}
            -b, etc.). New items need re-estimation and grooming.
          </DialogDescription>
        </DialogHeader>
              ...
              >
                ...
                <div className="mb-4 grid grid-cols-1 gap-4 md:grid-cols-3">
                  <div className="space-y-1">
                    <Label htmlFor={`split-id-${row._internalId}`}>
                      Backlog ID
                    </Label>
                    <Input
                  ...
                  <div className="space-y-1 md:col-span-2">
                    <Label htmlFor={`split-title-${row._internalId}`}>
                      Title*
                    </Label>
                    <Input
                  ...
                  <div className="space-y-1">
                    <Label htmlFor={`split-priority-${row._internalId}`}>
                      Priority*
                    </Label>
                    <Select
                      value={row.priority ?? 'Medium'}
                      onValueChange={(value) =>
                        handlePriorityChange(row._internalId, value)
                      }
                    >
                  ...
                  <div className="space-y-1">
                    <Label htmlFor={`split-sp-${row._internalId}`}>
                      Story Points
                    </Label>
                    <Input
                ...
                <div className="mb-4 space-y-1">
                  <div className="flex items-center justify-between">
                    <Label htmlFor={`split-desc-${row._internalId}`}>
                      Description
                    </Label>
                    <Button
                      variant="outline"
                      size="xs"
                      onClick={() => handleCopyDescription(row._internalId)}
                      disabled={!originalTask.description}
                ...
                <div className="space-y-1">
                  <div className="flex items-center justify-between">
                    <Label htmlFor={`split-ac-${row._internalId}`}>
                      Acceptance Criteria
                    </Label>
                    <Button
                      variant="outline"
                      size="xs"
                      onClick={() => handleCopyAC(row._internalId)}
                      disabled={!originalTask.acceptanceCriteria}
                      title="Copy original AC"
                    >
                      <Copy className="h-3 w-3" />
                    </Button>
                  </div>
        ...
        <DialogFooter className="mt-4">
          ...
          <Button
            ...
            disabled={splitTaskRows.length === 0}
          >
            Confirm Split
          </Button>
        </DialogFooter>
