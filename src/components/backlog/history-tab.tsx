...
import {
  ...
  Split,
  Undo,
} from 'lucide-react'; // Import icons for history status and Undo
...
type SortKey =
  ...
  | 'initiator'
  | 'needsGrooming'
  | 'readyForSprint'; // Added historyStatus
...
const getHistoryStatusIcon = (status: HistoryStatus | undefined | null) => {
  switch (status) {
    case 'Move':
      return <Move className="mr-1 inline h-3 w-3" />;
    case 'Split':
      return <Split className="mr-1 inline h-3 w-3" />;
    case 'Merge':
      return <GitMerge className="mr-1 inline h-3 w-3" />;
    default:
      return null;
  }
};
...
export default function HistoryTab({
  ...
  const sortedHistory = useMemo(() => {
    ...
    if (sortConfig !== null) {
      ...
      const statusOrder: (HistoryStatus | null)[] = [
        'Move',
        'Split',
        'Merge',
        null,
      ]; // Include null for items without status
      sortableItems.sort(
        (a, b) =>
          statusOrder.indexOf(a.historyStatus) -
            statusOrder.indexOf(b.historyStatus) ||
  ...
  const getSortIndicator = (key: SortKey) => {
    if (!sortConfig || sortConfig.key !== key) {
      return <ArrowUpDown className="ml-1 inline h-3 w-3 opacity-30" />; // Consistent style
    }
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <History className="h-5 w-5 text-primary" /> Backlog History
          </CardTitle>
          <CardDescription>
            View backlog items that have been moved, split, or merged for
            project '{projectName}'. Click headers to sort.
          </CardDescription>
        </CardHeader>
        <CardContent>
          {sortedHistory.length === 0 ? (
            <div className="flex min-h-[200px] flex-col items-center justify-center rounded-md border-2 border-dashed p-6">
              ...
              <p className="text-muted-foreground">
                No historical backlog items found.
              </p>
            </div>
          ) : (
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead className="w-[100px]">
                        {' '}
                        {/* Add History Status Header */}
                        <Button
                          variant="ghost"
                          onClick={() => requestSort('historyStatus')}
                          className="h-auto justify-start px-1 text-xs font-medium text-muted-foreground"
                        >
                      ...
                      <TableHead className="w-[100px]">
                        <Button
                          variant="ghost"
                          onClick={() => requestSort('movedToSprint')}
                          className="h-auto justify-start px-1 text-xs font-medium text-muted-foreground"
                        >
                  ...
                  <TableBody>
                    {sortedHistory.map((task) => (
                      <TableRow key={task.id}>
                        ...
                        <TableCell>
                          {task.createdDate &&
                          isValid(parseISO(task.createdDate))
                            ? format(parseISO(task.createdDate), 'MMM d, yyyy')
                            : 'N/A'}
                        </TableCell>
                        ...
                        <TableCell>
                          {task.historyStatus ? (
                            <Badge variant="outline" className="capitalize">
                              {getHistoryStatusIcon(task.historyStatus)}{' '}
                              {task.historyStatus}
                            </Badge>
                          ) : (
                            <span className="italic text-muted-foreground">
                        ...
                        {/* Undo Button Cell */}
                        <TableCell className="text-center">
                          {(task.historyStatus === 'Split' ||
                            task.historyStatus === 'Merge') && (
                            <Button
                              variant="ghost"
                              size="icon"
