...
export default function SprintSummaryTab({
  ...
  const getStatusBadgeVariant = (
    ...
    switch (status) {
      case 'Active':
        return 'default'; // Use primary color (blue)
      case 'Planned':
        return 'secondary'; // Use secondary color (gray)
      case 'Completed':
        return 'outline'; // Use outline style
      default:
        return 'secondary';
    }
  };
  ...
  const getStatusColorClass = (status: Sprint['status']): string => {
    switch (status) {
      ...
      case 'Completed':
        return 'text-green-600'; // Green (using direct Tailwind class for now)
      default:
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          <Eye className="h-5 w-5 text-primary" /> Sprint Summary: {projectName}
        </CardTitle>
        <CardDescription>
          Overview of all sprints for the project. Edit legacy details or delete
          sprints. For planning new sprints, use the 'Planning' sub-tab.
        </CardDescription>
      </CardHeader>
      <CardContent>
                      ...
                      <TableRow key={sprint.sprintNumber}>
                        ...
                        <TableCell className="space-x-1 text-center">
                          ...
                          <Link
                            ...
                            tabIndex={
                              sprint.status === 'Completed' ? -1 : undefined
                            }
                            onClick={(e) => {
                          ...
                          <AlertDialog>
                            <AlertDialogTrigger asChild>
                              <Button
                                ...
                                title="Delete Sprint"
                                className="h-8 w-8 text-destructive hover:bg-destructive/10 hover:text-destructive" // Smaller icon button, destructive color
                              >
