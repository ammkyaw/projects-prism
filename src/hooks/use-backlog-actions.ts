...
import {
  ...
  taskPriorities,
  initialBacklogTask,
} from '@/types/sprint-data'; // Add initialBacklogTask
...
export const useBacklogActions = ({
  ...
  const handleSaveNewBacklogItems = useCallback(
    (newItems: Task[]) => {
      if (!selectedProject) {
        toast({
          variant: 'destructive',
  ...
  const handleUpdateSavedBacklogItem = useCallback(
    (updatedItem: Task) => {
      if (!selectedProject) {
        toast({
          variant: 'destructive',
      ...
      const updatedBacklog = (selectedProject.backlog ?? []).map((item) =>
  ...
  const handleMoveToSprint = useCallback(
    (backlogItemId: string, targetSprintNumber: number) => {
      if (!selectedProject) {
        toast({
          variant: 'destructive',
          title: 'Error',
      ...
      const backlogItemIndex = (selectedProject.backlog ?? []).findIndex(
      ...
      const targetSprintIndex = (
        selectedProject.sprintData.sprints ?? []
      ).findIndex((s) => s.sprintNumber === targetSprintNumber);
      ...
      if (movedItemDetails) {
        toast({
          title: 'Item Moved',
          description: `Backlog item '${movedItemDetails}' moved to Sprint ${targetSprintNumber}. Marked in backlog history.`,
        });
      }
    },
    [selectedProject, updateProjectData, toast]
  ...
  const handleMoveSelectedBacklogItemsToSprint = useCallback(
    (backlogItemIds: string[], targetSprintNumber: number): Task[] => {
      // Returns the tasks to be added to the sprint plan
      if (!selectedProject) {
        toast({
          variant: 'destructive',
          title: 'Error',
      ...
      if (targetSprintIndex === -1) {
        toast({
          variant: 'destructive',
          title: 'Error',
          description: `Target Sprint ${targetSprintNumber} not found.`,
        });
        return [];
      }
      ...
      backlogItemIds.forEach((itemId) => {
        const itemIndex = updatedBacklog.findIndex(
          (item) => item.id === itemId
        );
        if (itemIndex !== -1) {
          ...
          if (originalItem.movedToSprint || originalItem.historyStatus) {
            console.warn(
              `Item ${originalItem.backlogId} already processed or in history, skipping move.`
            );
            return; // Skip this item
          }

          itemsMovedDetails.push(
            `${originalItem.backlogId} (${originalItem.title || 'No Title'})`
          );
          ...
          console.warn(
            `Backlog item with ID ${itemId} not found during move from backlog to sprint plan.`
          );
        }
      });
      ...
      if (tasksForSprintPlan.length === 0 && backlogItemIds.length > 0) {
        toast({
          variant: 'warning',
          title: 'No New Items Moved',
          description:
            'Selected items might have already been processed or were not found in the active backlog.',
        });
        return [];
      }
      ...
      const finalBacklog = updatedBacklog.sort(
        (a, b) =>
          taskPriorities.indexOf(a.priority!) -
            taskPriorities.indexOf(b.priority!) ||
          (a.backlogId ?? '').localeCompare(b.backlogId ?? '')
  ...
  const handleRevertTaskToBacklog = useCallback(
    (sprintNumber: number, taskId: string, taskBacklogId: string | null) => {
      if (!selectedProject) {
        toast({
          variant: 'destructive',
          title: 'Error',
      ...
      let targetSprintIndex = currentProject.sprintData.sprints.findIndex(
      ...
      let targetSprint = {
        ...currentProject.sprintData.sprints[targetSprintIndex],
      }; // Clone sprint
      let updatedNewTasks = [...(targetSprint.planning?.newTasks || [])];
      let updatedSpilloverTasks = [
        ...(targetSprint.planning?.spilloverTasks || []),
      ];
      ...
      if (taskIndexInNew !== -1) {
        ...
        let taskIndexInSpillover = updatedSpilloverTasks.findIndex(
          (t) => t.id === taskId
        );
        if (taskIndexInSpillover !== -1) {
          ...
          console.warn(
            `Task ID ${taskId} not found in Sprint ${sprintNumber} planning (new or spillover).`
          );
          showToast({
            variant: 'warning',
            title: 'Task Not Found in Sprint',
      ...
      const updatedBacklog = (currentProject.backlog || []).map((item) => {
        if (
          taskBacklogId &&
          item.backlogId === taskBacklogId &&
          item.movedToSprint === sprintNumber &&
          item.historyStatus === 'Move'
        ) {
      ...
      if (!updatePerformed) {
        console.warn(
          `Could not find corresponding original backlog item for task ${revertedTaskDetails} (Backlog ID: ${taskBacklogId}) that was marked as moved to sprint ${sprintNumber}. Task removed from sprint only.`
        );
        showToast({
          variant: 'warning',
          title: 'Task Removed from Sprint',
          description: `Task '${revertedTaskDetails}' removed from Sprint ${sprintNumber}. Its original backlog item was not found or not marked as 'Moved'.`,
        });
      } else {
        showToast({
          title: 'Task Reverted to Backlog',
          description: `Task '${revertedTaskDetails}' removed from Sprint ${sprintNumber} and restored to the active backlog.`,
        });
      }
  ...
  const handleSplitBacklogItem = useCallback(
    (originalTaskId: string, splitTasks: Task[]) => {
      if (!selectedProject) {
        toast({
          variant: 'destructive',
          title: 'Error',
      ...
      const originalBacklogIndex = (selectedProject.backlog ?? []).findIndex(
        (item) => item.id === originalTaskId
      );
      if (originalBacklogIndex === -1) {
        console.error(
          'Original backlog item not found for splitting:',
          originalTaskId
        );
        toast({
          variant: 'destructive',
  ...
  const handleDeleteSavedBacklogItem = useCallback(
    (itemId: string) => {
      if (!selectedProject) {
        toast({
          variant: 'destructive',
      ...
      const itemToDelete = (selectedProject.backlog ?? []).find(
        (item) => item.id === itemId
      );
      if (!itemToDelete) {
        toast({
          variant: 'destructive',
          title: 'Error',
          description: 'Item not found for deletion.',
        });
        return;
      }
      ...
      if (itemToDelete.historyStatus || itemToDelete.movedToSprint) {
        toast({
          ...
          title: 'Cannot Delete',
          description:
            'Historical or moved items cannot be deleted from here. Manage them in History or Sprint.',
        });
        return;
      }

      const updatedBacklog = (selectedProject.backlog ?? []).filter(
        (item) => item.id !== itemId
      );
      const updatedProject: Project = {
        ...selectedProject,
      ...
      toast({
        ...
        description: `Item '${itemToDelete.backlogId}' has been removed.`,
      });
    },
    [selectedProject, updateProjectData, toast]
  );
  ...
  const handleMergeBacklogItems = useCallback(
    (taskIdsToMerge: string[], mergedTask: Task) => {
      if (!selectedProject) {
        toast({
          variant: 'destructive',
          title: 'Error',
      ...
      currentBacklog.forEach((item) => {
        if (taskIdsToMerge.includes(item.id)) {
          if (item.historyStatus || item.movedToSprint) {
            ...
            console.warn(
              `Item ${item.backlogId} is already historical or moved, cannot merge.`
            );
            activeBacklogAfterProcessing.push(item); // Keep it as is
            return;
          }
          mergedItemDetails.push(
            `${item.backlogId} (${item.title || 'No Title'})`
          );
      ...
      if (itemsToMarkHistorical.length < 2) {
        toast({
          variant: 'destructive',
          title: 'Merge Error',
          description: 'Not enough valid items selected for merging.',
        });
        return;
      }
  ...
  const handleUndoBacklogAction = useCallback(
    (itemWithHistoryId: string) => {
      if (!selectedProject) {
        toast({
          variant: 'destructive',
          title: 'Error',
      ...
      const itemTriggeringUndo = (selectedProject.backlog || []).find(
        (item) => item.id === itemWithHistoryId
      );
      ...
      if (!itemTriggeringUndo) {
        toast({
          variant: 'destructive',
          title: 'Error',
          description: 'Item to undo not found.',
        });
        return;
      }
      ...
      if (itemTriggeringUndo.historyStatus === 'Split') {
        ...
        const splitChildren = updatedBacklog.filter(
          (task) => task.splitFromId === itemTriggeringUndo.id
        );
        if (splitChildren.length > 0) {
          ...
          const originalIndex = updatedBacklog.findIndex(
            (task) => task.id === itemTriggeringUndo.id
          );
          if (originalIndex !== -1) {
        ...
        const originalItemIndex = updatedBacklog.findIndex(
          (task) =>
            task.id === originalSplitItemId && task.historyStatus === 'Split'
        );
        ...
        if (originalItemIndex !== -1) {
          ...
          updatedBacklog = updatedBacklog.filter(
            (task) => task.splitFromId !== originalSplitItemId
          );
          // Restore original
        ...
        if (eventId) {
          const itemsFromMergeEvent = updatedBacklog.filter(
            (task) => task.mergeEventId === eventId
          );
          const resultingMergedItem = itemsFromMergeEvent.find(
            (task) => !task.historyStatus
          ); // The one that is NOT historical
          const originalMergedItems = itemsFromMergeEvent.filter(
            (task) => task.historyStatus === 'Merge'
          );

          if (resultingMergedItem && originalMergedItems.length > 0) {
            // Remove the resulting merged item
            updatedBacklog = updatedBacklog.filter(
              (task) => task.id !== resultingMergedItem.id
            );
        ...
        itemTriggeringUndo.mergeEventId &&
        !itemTriggeringUndo.historyStatus
      ) {
        // This is the resulting merged item. We need to find the originals.
        const eventId = itemTriggeringUndo.mergeEventId;
        const originalMergedItems = updatedBacklog.filter(
          (task) =>
            task.mergeEventId === eventId && task.historyStatus === 'Merge'
        );
